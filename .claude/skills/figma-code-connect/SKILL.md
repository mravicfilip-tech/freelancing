...
> **Local copy of Figma's official skill.** Installed from the Figma MCP server (`skill://figma/figma-code-connect/SKILL.md`). The `references/` files it links are NOT vendored here — read them live with the `get_figma_skill` tool using `skill://figma/figma-code-connect/references/<path>` (e.g. `references/api.md`, `references/advanced-patterns.md`).
