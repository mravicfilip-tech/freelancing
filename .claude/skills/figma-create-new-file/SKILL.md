...
> **Local copy of Figma's official skill.** Installed from the Figma MCP server (`skill://figma/figma-create-new-file/SKILL.md`). Any `references/` files it links are NOT vendored here — read them live with the `get_figma_skill` tool using `skill://figma/<skill-name>/references/<path>`. Cross-skill links like `../figma-use-slides/SKILL.md` resolve to the sibling skill directory here.
