...
> **Local copy of Figma's official skill.** Installed from the Figma MCP server (`skill://figma/figma-generate-design/SKILL.md`). The `references/` files it links are NOT vendored here — read them live with the `get_figma_skill` tool using `skill://figma/figma-generate-design/references/<path>` (e.g. `references/discover-product-font.md`, `references/componentization.md`). Links into `../figma-use/references/...` resolve to `skill://figma/figma-use/references/<path>`. Cross-skill `SKILL.md` links resolve to the sibling skill directory here.
