...
> **Local copy of Figma's official skill.** Installed from the Figma MCP server (`skill://figma/figma-generate-diagram/SKILL.md`). The `references/` files it links are NOT vendored here — read them live with the `get_figma_skill` tool using `skill://figma/figma-generate-diagram/references/<path>` (e.g. `references/architecture.md`, `references/flowchart.md`, `references/workflow.md`). Cross-skill links like `../figma-use-figjam/SKILL.md` resolve to the sibling skill directory here.
