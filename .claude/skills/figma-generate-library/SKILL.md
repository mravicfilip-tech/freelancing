...
> **Local copy of Figma's official skill.** Installed from the Figma MCP server (`skill://figma/figma-generate-library/SKILL.md`). The `references/` and `scripts/` files it links are NOT vendored here — read them live with the `get_figma_skill` tool using `skill://figma/figma-generate-library/references/<path>` or `skill://figma/figma-generate-library/scripts/<file>`. Cross-skill links like `../figma-use/SKILL.md` resolve to the sibling skill directory here.
