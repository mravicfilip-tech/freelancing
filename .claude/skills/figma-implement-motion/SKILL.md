...
> **Local copy of Figma's official skill.** Installed from the Figma MCP server (`skill://figma/figma-implement-motion/SKILL.md`). The `references/` files it links are NOT vendored here — read them live with the `get_figma_skill` tool using `skill://figma/figma-implement-motion/references/<path>` (e.g. `references/gotchas.md`, `references/motion-lint-rules.md`). Cross-skill links like `../figma-use/SKILL.md` resolve to the sibling skill directory here.
