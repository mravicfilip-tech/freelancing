...
> **Local copy of Figma's official skill.** Installed from the Figma MCP server (`skill://figma/figma-use-slides/SKILL.md`). The `references/` files it links are NOT vendored here — read them live with the `get_figma_skill` tool using `skill://figma/figma-use-slides/references/<path>` (e.g. `references/slide-gotchas.md`, `references/slide-design.md`). Cross-skill links like `../figma-use/SKILL.md` resolve to the sibling skill directory here.
