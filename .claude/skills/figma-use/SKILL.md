...
> **Local copy of Figma's official skill.** Installed from the Figma MCP server (`skill://figma/figma-use/SKILL.md`). The `references/` and `scripts/` files it links are NOT vendored here — read them live with the `get_figma_skill` tool using `skill://figma/figma-use/references/<path>`. Cross-skill links like `../figma-generate-design/SKILL.md` resolve to the sibling skill directory here.
