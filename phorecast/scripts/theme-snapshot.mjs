  ...
  for (const sel of SECTIONS) {
    forWidth[sel] = await page.evaluate(([s, props]) => {
      ...
      for (const el of [...root.querySelectorAll('*')].slice(0, 1200)) {
        ...
        // Long inline data: URIs (the WebGL mark's static stand-in is 5 KB of
        // base64, repeated per width) are reduced to a stable fingerprint.
        // A swapped asset still shows as a difference; the file stays a
        // readable size, and the baseline is committed on every agent.
        const src = el.getAttribute('src') ?? '';
        row.src = src.length <= 100 ? src
          : `${src.slice(0, 24)}…${src.length}#${(() => {
              let h = 5381;
