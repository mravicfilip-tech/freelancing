 ...
 *
 * LOAD-IN (about 2.5s, after the band's entrance has landed the card)
 *   The measure grid arrives with the card. The chart line leads: a 1.0s draw,
 *   tip to tail, on `power2.inOut`. The marker appears as the drawing edge
 *   passes, the pie and lightning badges follow, the pill rises last and both
 *   figures tally to $200.00. Keep it short: it plays only after the band's
 *   entrance has finished.
 *
 * LOOP (5.3s of motion, then 4.4s still: 9.7s end to end)
 *   The claim is growth, so the loop is the line growing. After a beat the
 *   curve retracts right to left in half a second, then redraws over 2.2s with
 *   a lit dot riding the drawing edge. The marker blooms as the edge reaches
 *   it, the badges and the pill lift, and the bonus figure counts up to $200.00
 *   beside the deposit. The card then rests on exactly the design's artwork.
 *
 * THE CROP (why the draw is not simply "the whole path")
 *   The landscape card hangs the chart well off its right edge only, so almost
 *   all the curve is on the card. The phone frame (Figma 526:261) hangs it 110
 *   design px off the left and 61 off the right, putting roughly a third of
 *   the path off the card. So the draw is measured against the card's clip and
 *   runs over the visible arc only, with the same beats and durations. Where
 *   the crop is under EDGE_TOL (the landscape card) the window snaps to the
 *   full path.
 *
 * Nothing is re-parented or cloned out of the `<svg>`: the line's
 * `stroke="url(#box-bonus-stroke)"` resolves against that svg's own `<defs>`,
 * and moving either out of the subtree would drop the gradient silently. The
 * added dot lives in the same svg with a flat fill.
 */
 ...
 *  fraction of its length. The landscape card crops under 2% (not worth
 *  retiming); a phone crops 11% to 20% at each end. */
const EDGE_TOL = 0.03;
...
export function bonus(card: HTMLElement): () => void {
  ...
  /* Build-time colour reads; see the note in funds.ts. The "lit" values invert
     on paper (see the light block in Bento.css). */
  const C = {
  ...
  /* The visible arc. Two boxes clip this curve (the card and the artwork well
     inside it), so the window is their intersection, measured from the DOM
     rather than keyed to a breakpoint. Both edges snap to the ends of the path
     when the crop is under EDGE_TOL, so the landscape card gives exactly 0 and
     `total`. */
  ...
  const ctx = gsap.context(() => {
    ...
    const DRAW_AT = 0.8;
    const DRAW = 2.2;
    /* Every `fromTo` below states `immediateRender: false`. A `fromTo` renders
       its from value on creation, even inside a paused timeline; here that
       would draw the line fully before the load-in, which then tweens nothing. */
