...
/** `?slide=3` opens on a given slide (handy for review). It does not pause
 *  autoplay, so a shared link to a slide behaves like the page does. To hold a
 *  slide for review, press the play control. */
...
export function Hero() {
  ...
  /*
   * WHAT HOLDS THE CAROUSEL.
   *
   * Not hover, and not a pointer's focus. On a laptop the hero IS the
   * viewport, so a cursor resting anywhere on the first screen would stop it
   * for good, and a click on a pager segment is a request to change slide,
   * not to stop the carousel.
   *
   * What does:
   ...
   * and the slide changing stay the same moment. (Restarting a full 7s on
   * every release would let the track run out seconds before the slide
   * changes.)
   *
  ...
  // Slides 1 and 4 show the mark: slide 1 as its illustration, and slide 4 on
  // `.sl4__mark-slot` (see SLOT_SLIDE above). One scene serves both.
  ...
  // Off screen, the carousel holds. A slide change is not a cheap thing to do
  // unwatched: it re-renders the stage, swaps the background modifier, and
  // builds a fresh slideIn (a masked line reveal with blur across the display
  // type and the lede, plus the illustration's pop).
  //
  ...
  // copy from zero, which looks exactly like the entrance stuttering.
  ...
  useEffect(() => {
    ...
    // timeline touched. Killing the timeline and clearing props from a
    // hand-written selector list would leave anything not on the list (the
    // illustration's parts, above all) frozen at the start values `from` had
    // written, such as an illustration stuck at 92% or a button stranded at
    // opacity 0. revert() restores what GSAP set, all of it, by construction.
    let guard = 0;
  ...
  // SMALLER of `heightFraction * h` and `widthFraction * w`, and a 0.7 width
  // fraction would win and draw a small mark sitting low in its box. 0.94
  // takes width out of the way and hands the decision to the box.
  //
  // 0.88 and not 0.96, because `size` is not the mark's drawn height: the
  // silhouette is about 1.084 times taller than the number it is given. At
  // 0.96 the mark overflows the 315.8px box at 390 and `.heroLogo`'s
  // `overflow: hidden` clips it; 0.88 draws it at about 301px with 7px of air
  // either side.
  //
  // Two columns, `cy` is a fraction of the hero. 0.42 puts the mark's middle
  // on the copy's middle at laptop sizes (1280 to 1512), with clear air under
  // the nav. The width fraction binds at those widths, so it sets the size.
   ...
   * WebGLRenderer and build a fresh one on every carousel advance, for the
   * life of the page, none of it visible in a screenshot. With the transform
   * below the scene is built once.
   *
  ...
  useLayoutEffect(() => {
    ...
    const place = () => {
      ...
      // now and the slot's rect is off by the same amount. `.hero__stage` is the
      // visual's containing block and is never transformed, so the visual's own
      // scale is the ratio of the two widths; divide it out and the slot is back
      // on the coordinates CSS states for it, whenever this is asked.
    ...
    // container's size at the last layout rather than the current one. So the
    // slot can settle a frame after the hero (for example once a reserved
    // scrollbar gutter goes away), and watching the hero alone could leave the
    // mark slightly small for the life of the page. The slot's box settling is
  ...
  const mark = (
    <HeroLogo
      ...
      className={[
        ...
        // when the slide changed, that is, only when somebody could
      ...
      <div className="container container--wide hero__inner">
        ...
        <div className="hero__position">
          <Position
            ...
            // would resume the elapsed time banked when the reader scrolled
            // away and fill to the end seconds before the slide it is
            // describing actually changes.
            cycleKey={onScreen}
          />
