...
/**
 * Entrance for the audits band, built like the FAQ's: the heading rises out of its mask, the
 * gutter rule draws down between the cells, and the two cells arrive in reading order.
 */
    ...
    Promise.all([import('gsap'), import('gsap/ScrollTrigger')])
      .then(([{ gsap }, { ScrollTrigger }]) => {
        ...
        const ctx = gsap.context(() => {
          ...
          tl.from(el.querySelector('.av__gutter'), { scaleY: 0, duration: 1.1, ease: 'expo.inOut' }, 0.25);
          tl.from(el.querySelectorAll('.av__cell'), { opacity: 0, y: 18, duration: 0.75, ease: 'expo.out', stagger: 0.1 }, 0.45);
